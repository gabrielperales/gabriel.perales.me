...
- **Analytics**: Piqo (`https://piqo.app`), not one of pliny's built-in providers, so it's injected manually as a `next/script` tag in `app/layout.tsx` (not via pliny's `<Analytics>` component). Its `data-site` id lives in `data/siteMetadata.js` under `analytics.piqoAnalytics.dataSite`; `data-domain` is derived from `siteMetadata.siteUrl`. `piqo.app` must stay listed in the CSP `script-src` in `next.config.js`.
